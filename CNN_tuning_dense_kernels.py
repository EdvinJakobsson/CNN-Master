...
dropoutfolder = 'Results/tuning_hypers/linear-model4/essayset' + str(essayset[0]) + '/dropout' + str(dropout) + '/'
...
for dense in dense_numbers:
    ...
    f = open(file, "w+")
    f.write("essayset: " + str(essayset[0]) + " \t \t epochs: " + str(numbers_of_kappa_measurements*epochs_between_kappa) + " \t \t  Dropout: " + str(dropout) + " \t \t k-fold: no \t \t batch size: 10 \t\t layers: 2 \t \t output: " + str(output) + " \t \t dense: " + str(dense) + " \r \r")
    ...
    for kernel_length in kernel_length_number:
        ...
        for kernels in kernel_numbers:
            ...
            x_train, d_train, x_val, d_val = functions.split_data(pad_seq, essaysetlist, essaynumber, targets, VALIDATION_SPLIT)
            ...
            max_val_kappa = -1
            epoch = 0
            path = dropoutfolder + "/training-values/Dense" + str(dense) + "Kernels" + str(kernels) + "Length" + str(kernel_length)
            ...
            matrix_savefile = matrix_folder + "dense" + str(dense) + "kernels" + str(kernels) + "kernellength" + str(kernel_length) + "epochs" + str(epochs_between_kappa * numbers_of_kappa_measurements)
            ...
            plotimage = graph_folder + "Dense" + str(dense) + "Kernels" + str(kernels) + "Length" + str(kernel_length) + ".png"
