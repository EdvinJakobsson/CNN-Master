...
from cnn_master.ben_hamner.score import mean_quadratic_weighted_kappa, quadratic_weighted_kappa
from cnn_master import reader_full
...
def process_texts(data, output, essays, asap_ranges, human_range):
    ...
    for row in data:
        ...
        if human_range == False:
            targets.append(
                int(row[6]) - range[0]
            )  # -range changes grades to start at 0
        else:
            targets.append(
                int(row[3]) - range[0]
            )  # -range changes grades to start at 0
    ...
    if output == "softmax":
        targets = to_categorical(
            np.asarray(targets), number_of_classes
        )  # creates a target vector for each text. If a text belongs to class 0 out of 4 classes the vector will be: [1., 0., 0., 0.]
...
def texts_to_sequences(MAX_NUM_WORDS, texts):
    ...
    sequences = tokenizer.texts_to_sequences(
        texts
    )  # list of all texts where words are numbers instead
    word_index = (
        tokenizer.word_index
    )  # dictionary mapping each word to the correct number
...
def split_data(
    pad_sequences, essayset, essaynumber, targets, VALIDATION_SPLIT, randomize_data
):

    indices = np.arange(
        pad_sequences.shape[0]
    )  # creates an array with integers up to the total number of texts (data.shape[0]). ex: [0  1  2  3 ... 1998  1999]
    if randomize_data:
...
def save_confusion_matrix(
    savefile, x, d, model, essayset, output, asap_ranges, title=None
):
