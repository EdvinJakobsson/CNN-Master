...
human_range = (
    False
)  # set to true if comparison should be made with one expert human grader instead of the resolved score of the two experts
if human_range == True:
...
# hyper-parameters
output = "hybrid"  # linear, sigmoid, softmax or hybrid
model_number = 1
dropout = 0.5
...
for test in range(1, tests + 1):
    ...
    for essayset in essaysets:
        ...
        set = essayset[0]
        number_of_kappa_measurements = 1
        batch_size = 40
        if set == 7:
        ...
        for i in range(1, number_of_kappa_measurements + 1):
            ...
            model.fit(
                ...
                verbose=False,
                validation_data=([x_val, x_val_mlp], d_val),
            )
            train_loss, train_acc = model.evaluate(
                [x_train, x_train_mlp], d_train, verbose=2
            )
            val_loss, val_acc = model.evaluate([x_val, x_val_mlp], d_val, verbose=2)
            train_kappa = functions.quadratic_weighted_kappa_for_hybrid(
                [x_train, x_train_mlp], d_train, essayset, model, output
            )
            val_kappa = functions.quadratic_weighted_kappa_for_hybrid(
                [x_val, x_val_mlp], d_val, essayset, model, output
            )
